...
describe('GET /todos/:id', () => {
	...
	it('should not return todo doc created by other user', (done) => {
		request(app)
		.get(`/todos/${todos[1]._id.toHexString()}`)//toHexString() convert objectID to a string to be passed into URL
		.set('x-auth', users[0].tokens[0].token)
		.expect(404)
		.end(done);
	});
	...
	it('should create a user', (done) => {
		...
		request(app)
			...
			.expect((res) => {
				...
				expect(res.body.email).toBe(email); //since we know the email already
			})
			...
			.end((err) => {
				...
				User.findOne({email}).then((user) => {
					expect(user).toExist();
					expect(user.password).toNotBe(password); //since it's hashed
					done();
				}).catch((e) => done(e));
			});
	});
